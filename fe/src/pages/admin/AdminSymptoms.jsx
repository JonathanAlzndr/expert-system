...
const AdminSymptoms = () => {
	...
	const updateGejala = async (idGejala, data) => {
		try {
			...
			return {
				success: false,
				error: err.response?.data?.msg || "Failed to update gejala",
			};
		}
	};
	...
	const handleDelete = async (idGejala) => {
		if (
			window.confirm(
				"Apakah Anda yakin ingin menghapus gejala ini? Tindakan ini juga akan menghapus pertanyaan terkait."
			)
		) {
	...
	const handleEdit = (gejala) => {
		setFormData({
			...
			teks_pertanyaan: gejala.teks_pertanyaan,
		});
