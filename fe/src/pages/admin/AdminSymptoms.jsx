...
const AdminSymptoms = () => {
    ...
    // 2. PANGGIL HOOK dengan parameter page
    const { 
        gejala, 
        meta, // Ambil meta
        loading, 
        error, 
        createGejala, 
        updateGejala, // Gunakan dari hook langsung (sudah include refresh)
        deleteGejala 
    } = useGejala(currentPage, itemsPerPage);
    ...
    // --- 3. LOGIKA PAGINATION SERVER SIDE ---
    // HAPUS SLICING DI SINI! Data 'gejala' dari hook sudah 10 item.
    // const currentGejala = gejala.slice(...) <-- HAPUS INI

    // Ambil total pages dari META backend
    ...
    return (
        <>
            ...
            {/* Modal tetap sama */}
            <Modal isOpen={showForm} onClose={handleCloseModal} title={editingGejala ? "Ubah Gejala" : "Tambah Gejala Baru"}>
                {/* ... isi modal sama ... */}
                {formError && (<div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">{formError}</div>)}
                <form onSubmit={handleSubmit}>
                    <FormInput label="ID Gejala" name="id_gejala" value={formData.id_gejala} onChange={handleInputChange} placeholder="Contoh: G01" required disabled={!!editingGejala} />
                    <FormInput label="Nama Gejala" name="nama_gejala" value={formData.nama_gejala} onChange={handleInputChange} placeholder="Masukkan nama gejala" required />
                    <div className="mb-6">
                        <label className="block text-gray-700 mb-2">Teks Pertanyaan</label>
                        <textarea name="teks_pertanyaan" value={formData.teks_pertanyaan} onChange={handleInputChange} rows="3" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Masukkan teks pertanyaan untuk diagnosis" required ></textarea>
                    </div>
                    <div className="flex justify-end">
                        <Button type="button" variant="secondary" className="mr-2" onClick={handleCloseModal}>Batal</Button>
                        <Button type="submit" disabled={submitLoading}>{submitLoading ? "Menyimpan..." : editingGejala ? "Perbarui" : "Simpan"}</Button>
                    </div>
                </form>
            </Modal>
        </>
